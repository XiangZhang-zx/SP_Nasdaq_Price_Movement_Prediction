...
# 设置文件路径
file_path_ixic = '/usr3/graduate/xz0224/co（就是从close买，明天的open卖）/ixic.csv'
file_path_cash = '/usr3/graduate/xz0224/co（就是从close买，明天的open卖）/cash.csv'

# 读取数据
data_ixic = pd.read_csv(file_path_ixic)
data_cash = pd.read_csv(file_path_cash)

# 确保日期列格式为datetime
data_ixic['date'] = pd.to_datetime(data_ixic['date'])
data_cash['date'] = pd.to_datetime(data_cash['date'])

# 创建 return_label 列
data_ixic['return_label'] = data_ixic['return'].apply(lambda x: '+' if x > 0 else '-')
...
data_ixic['feature'] = create_label_sequence(data_ixic)
data_cash['feature'] = create_label_sequence(data_cash)

# 合并特征和目标变量，只保留ixic的特征
combined_data = pd.merge(data_ixic, data_cash, on='date')
combined_data['combined_feature'] = combined_data['feature_x']  # 只使用ixic的特征
combined_data['target'] = combined_data.apply(lambda row: 'ixic' if row['return_x'] > row['return_y'] else 'cash', axis=1)
...
file_path = '/usr3/graduate/xz0224/co（就是从close买，明天的open卖）/ixic/investment_results_final(2006)(train_3month_c-c).xlsx'
...
while current_date <= end_date:
    ...
    # 设置训练集的时间范围（前五年）
    train_start = current_date - pd.DateOffset(months=3)
    ...
    if not test_set.empty:
        ...
        for i, decision in enumerate(decisions):
            ...
            ixic_rate = data_ixic.loc[data_ixic['date'] == date, 'return'].values
            ...
            # 记录日期、金额和决策
            sheet.append([date, current_value, 'ixic' if decision == 1 else 'cash'])
            wb.save(file_path)
